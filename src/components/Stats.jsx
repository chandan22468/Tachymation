...
const AnimatedCounter = ({ value, suffix, isDecimal }) => {
  ...
  return (
    <span ref={counterRef} className="font-space-grotesk font-bold text-5xl md:text-7xl text-white">
      {displayValue}{suffix}
    </span>
  );
};
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="stat-card bg-[#121212] rounded-lg p-6 md:p-8 text-center border border-white/10 shadow-sm"
            >
              <div className="mb-3">
                <AnimatedCounter 
                  value={stat.value} 
              ...
              <p className="font-space-grotesk font-bold text-lg text-white mb-1">
                {stat.label}
              </p>
